"""``atria-module`` — scaffold and locally develop Atria service modules.

    atria-module new <name> [--summary "…"] [--port 9300]
...
The backend serves the connector on the module's port; point Atria at it with the
manifest's ``connector_url`` (use ``http://localhost:<port>`` for host-run Atria).
"""
...
def _repo_root() -> Path:
    # atria/module_dev.py -> repo root is two parents up.
...
def cmd_new(args: argparse.Namespace) -> int:
    ...
    print("Next:")
    print(f"  atria-module dev {args.name}        # run backend + dashboard locally")
...
def cmd_dev(args: argparse.Namespace) -> int:
    ...
    sdk_path = _repo_root() / "atria_module_sdk"

    env = os.environ.copy()
    # SDK on PYTHONPATH so ``import atria_module_sdk`` works without installing;
    ...
    print(f"[atria-module] backend  → uvicorn app:app --reload --port {port}")
    ...
    if (frontend / "package.json").is_file() and not args.no_frontend:
        print(f"[atria-module] frontend → npm run dev  (in {frontend})")
        try:
            procs.append(subprocess.Popen(["npm", "run", "dev"], cwd=str(frontend), env=env))
        except FileNotFoundError:
            print("[atria-module] npm not found; skipping frontend dev server.",
                  file=sys.stderr)

    print(f"[atria-module] connector at http://localhost:{port}/connector/health  (Ctrl-C to stop)")
    ...
    try:
        ...
        while True:
            for p in procs:
                code = p.poll()
                if code is not None:
                    print(f"[atria-module] a process exited (code {code}); shutting down.")
                    _stop()
                    for q in procs:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog="atria-module",
        description="Scaffold and locally develop Atria service modules.")
