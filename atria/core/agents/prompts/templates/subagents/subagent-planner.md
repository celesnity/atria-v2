...
- Start the app (`atria` or `npm run dev` etc.) and exercise the changed feature
...
When your plan is written to the file, call **task_complete** with a brief summary that includes the plan_file_path (e.g., "Plan written to ~/.atria/plans/add-auth.md. Approach: ...").
