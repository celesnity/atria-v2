"""FastAPI web server for Minder UI."""
...
from minder.web.websocket import websocket_endpoint
from minder.web.transcribe_ws import transcribe_ws_endpoint
...
from minder.core.context_engineering.history import SessionManager, UndoManager
...
async def _bus_handler(topic: str, payload: dict) -> None:
    ...
    - ``minder:block:<sid>``: re-broadcast the WS envelope to all local clients;
      the chat store on each client filters by ``session_id``.
    - ``minder:event:<bid>``: invoke the locally-registered ``on_event`` handler
      if this worker is the one that called ``push_block``.
    """
    from minder.web.websocket import ws_manager as _ws
    from minder.web.ui_bridge import get_block_event_handler

    if topic.startswith("minder:block:"):
        await _ws.broadcast(payload)
    elif topic.startswith("minder:event:"):
        bid = topic.removeprefix("minder:event:")
...
async def lifespan(app: FastAPI):
    ...
    from minder.web.websocket import ws_manager as _global_ws_manager
    ...
    from minder.db.connection import init_schema, get_sessionmaker
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="Minder Web UI",
        description="Web interface for Minder AI coding assistant",
        version="0.1.0",
        lifespan=lifespan,
    )
...
def start_server(
    ...
    def run_server():
        try:
            ...
            os.environ.setdefault("MINDER_API_BASE", f"http://{api_host}:{port}")
            uvicorn.run(
                app,
                host=host,
                port=port,
