/*! MinderDash + MinderBlock host bridge. Loaded inside the dashboard iframe.
...
(function () {
  ...
  var MinderDash = {
    ...
    json: function (script, args, opts) {
      return MinderDash.run(script, args, opts).then(function (res) {
        if (res.exit_code !== 0) {
  ...
  var MinderBlock = {
    onTheme:  MinderDash.onTheme,
    onProps:  onify("props"),
    ready:    MinderDash.ready,
    resize:   MinderDash.resize,
  ...
  window.MinderDash = MinderDash;
  window.MinderBlock = MinderBlock;
})();
