...
from minder.web.protocol import WSMessageType, ws_message
...
async def send_query(
    ...
    from minder.web.websocket import ws_manager as global_ws_manager
    ...
    try:
        ...
        session_id = request.sessionId or await state.get_current_session_id()
        if not session_id:
            raise HTTPException(
                status_code=400,
        ...
        try:
            ...
            if (
                ...
                and (current.owner_id is None or current.owner_id == str(user.id))
            ):
                session = current
            else:
                raise HTTPException(
...
async def clear_chat(request: ClearChatRequest | None = None) -> Dict[str, str]:
    ...
    try:
        ...
        if request and request.workspace:
            await state.session_manager.create_session(working_directory=request.workspace)
        else:
