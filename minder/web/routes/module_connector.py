...
ever editing Minder core.

    GET|POST  /api/modules/{name}/connector/{path}   -> forwards to the connector
    GET       /api/modules/{name}/health             -> health + capabilities

The acting user's identity is forwarded to the connector as ``X-Minder-Principal``
(and a module-scoped secret as ``X-Minder-Module-Token`` if configured) by
``RemoteConnector``.
"""
...
from minder.core.modules.watcher import kick_reconcile
