...
to whatever OpenAI-compatible transcription backend ``MINDER_ASR_BASE_URL`` points
...
def _asr_config() -> tuple[str, str, str | None]:
    ...
    base_url = os.environ.get("MINDER_ASR_BASE_URL", _DEFAULT_BASE_URL).rstrip("/")
    model = os.environ.get("MINDER_ASR_MODEL", "nvidia/nemotron-3.5-asr-streaming-0.6b")
    api_key = os.environ.get("MINDER_ASR_API_KEY") or None
