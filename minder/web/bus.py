...
- ``minder:block:<session_id>`` — server→iframe (push / update / remove). Payload
  is the WS envelope the iframe expects: ``{"type": "...", "data": {...}}``.
- ``minder:event:<block_id>`` — iframe→Python ``on_event``. Payload is
  ``{"name": str, "data": Any}``.
...
- :class:`RedisBus`: ``psubscribe("minder:*")``; the canonical
...
class MessageBus(abc.ABC):
    ...
    def subscribe(self, handler: Handler) -> None:
        """Register a handler called for every received message on ``minder:*``.
...
class RedisBus(MessageBus):
    """Redis-backed pub/sub using ``psubscribe('minder:*')``."""
    ...
    async def start(self) -> None:
        ...
        self._task = asyncio.create_task(self._reader_loop(), name="minder-bus-reader")
