...
from minder.core.utils.tool_result_summarizer import summarize_tool_result
from minder.core.utils.tool_display import (
...
class WebSocketToolBroadcaster:
    ...
    def _activity_for(self, tool_name: str, arguments: Dict[str, Any]) -> Optional[Dict[str, str]]:
        ...
        if not command:
            return None
        try:
            from minder.core.modules.activity import resolve_activity_label
    ...
    def _sandbox_check(self, tool_name: str, arguments: Dict[str, Any]) -> Optional[str]:
        ...
        if not raw_path:
            return None
        from minder.core.workspace.manager import is_within_workspace
        ...
        # Always allow reads/writes inside the minder scratch/data directory
        minder_home = minder_dir()
        if resolved.is_absolute() and str(resolved).startswith(str(minder_home)):
            return None
