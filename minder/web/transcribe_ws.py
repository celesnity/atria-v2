...
from ``MINDER_ASR_BASE_URL``) and relays ``partial``/``final`` transcript JSON
...
def _asr_realtime_url() -> str:
    """Derive the sidecar's realtime WS URL from MINDER_ASR_BASE_URL."""
    base = os.environ.get("MINDER_ASR_BASE_URL", _DEFAULT_BASE_URL).rstrip("/")
...
async def transcribe_ws_endpoint(websocket: WebSocket) -> None:
    ...
    # Soft cookie auth, same pattern as websocket_endpoint (minder/web/websocket.py).
