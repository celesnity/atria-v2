...
from minder.core.auth.keycloak.jwt import InvalidTokenError
from minder.core.auth.keycloak.principal import (
    PrincipalResolutionError,
    build_principal,
)
from minder.models.user import User
from minder.web.routes.auth import TOKEN_COOKIE, verify_token
...
async def _resolve_via_bearer(request: Request) -> User:
    ...
    tenant = request.headers.get("X-Minder-Tenant", "").strip()
    if not tenant:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, "X-Minder-Tenant header required")
...
async def _resolve_via_session_cookie(request: Request, token: str) -> User | None:
    """Decode minder_session cookie and look up the user. Returns None on failure."""
    try:
