...
class ConfigManager:
    ...
    def load_config(self) -> AppConfig:
        ...
        1. Local project config (.minder/settings.json)
        2. Global user config (~/.minder/settings.json)
        3. Default values
        """
        # Start with defaults
        config_data: dict = {}
        ...
        # MINDER_MODEL / MINDER_FALLBACK_MODEL / MINDER_API_BASE_URL take effect.
        env_overrides = {
            "model": os.environ.get("MINDER_MODEL"),
            "fallback_model": os.environ.get("MINDER_FALLBACK_MODEL"),
            "api_base_url": os.environ.get("MINDER_API_BASE_URL"),
            "agent_mode": os.environ.get("MINDER_AGENT_MODE"),
        }
    ...
    def ensure_directories(self) -> None:
        ...
        # Expand paths
        minder_dir = Path(config.minder_dir).expanduser()
    ...
    def load_context_files(self) -> list[str]:
        """Load MINDER.md context files hierarchically.
        ...
        project_context = self.working_dir / "MINDER.md"
        ...
        while current != current.parent:
            subdir_context = current / "MINDER.md"
    ...
    def user_skills_dir(self) -> Path:
        """Get user-global skills directory (~/.minder/skills/)."""
        return get_paths(self.working_dir).global_skills_dir

    @property
    def project_skills_dir(self) -> Path | None:
        """Get project-local skills directory (<project>/.minder/skills/).
    ...
    def get_skill_dirs(self) -> list[Path]:
        ...
        1. Project skills (.minder/skills/)
    ...
    def _get_bundle_skill_dirs(self) -> list[Path]:
        ...
        bundle_dirs = []
        try:
            from minder.core.plugins import PluginManager
    ...
    def load_custom_agents(self) -> list[dict[str, Any]]:
        ...
        1. ~/.minder/agents.json (user global JSON)
        2. ~/.minder/agents/*.md (user global markdown)
        3. <project>/.minder/agents.json (project local JSON)
        4. <project>/.minder/agents/*.md (project local markdown)
