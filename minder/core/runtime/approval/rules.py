...
Persistent rules are stored in:
  - User-global: ~/.minder/permissions.json
  - Project-scoped: .minder/permissions.json
"""
...
class ApprovalRulesManager:
    ...
    USER_PERMISSIONS_PATH = minder_dir() / "permissions.json"
    ...
    def add_persistent_rule(self, rule: ApprovalRule, *, scope: str = "user") -> None:
        ...
        Args:
            ...
            scope: "user" for ~/.minder/permissions.json,
                   "project" for .minder/permissions.json in the project dir.
        """
        self.add_rule(rule)
    ...
    def clear_persistent_rules(self, *, scope: str = "user") -> int:
        ...
        if scope in ("project", "all") and self._project_dir:
            project_path = Path(self._project_dir) / ".minder" / "permissions.json"
    ...
    def _save_persistent_rules(self, *, scope: str = "user") -> None:
        ...
        if scope == "user":
            ...
            path = Path(self._project_dir) / ".minder" / "permissions.json"
        else:
            return
