...
from minder.core.modules.store_templates import (
    _GENERIC_DATA_ICON,
    _GENERIC_DATA_SCRIPT,
    _data_manifest_json,
    _data_skill_md,
    _starter_dashboard_html,
    _starter_main_script,
    _starter_manifest_json,
    _starter_skill_md,
)
...
# sortable/filterable table with row/column KPIs. Uses the MinderDash bridge.
...
<body>
  ...
  (function () {
    ...
    function resize(){ try { MinderDash.resize(document.body.scrollHeight + 24); } catch(e){} }
    ...
    function loadDataset(file, filter){
      ...
      return MinderDash.json('data.py', args).then(function(res){
        current = res; sortCol=-1; sortDir=1; applyView();
      }).catch(function(){ current={columns:[],rows:[]}; renderTable(); });
    }

    function refresh(){
      return MinderDash.json('data.py', ['list','--json']).then(function(res){
        ...
        try { MinderDash.setBadge(datasets.length || null); } catch(e){}
        if (datasets.length){
    ...
    if (window.MinderDash){
      MinderDash.ready();
      MinderDash.onChange(function(){ refresh(); });
      MinderDash.onVisibility(function(v){ if(v) resize(); });
    }
    refresh();
  })();
  </script>
...
def module_protected_paths(modules: List[Module]) -> list:
    ...
    """
    from minder.models.config import ProtectedPath  # local import: avoid import cycle
...
def create_module(
    ...
    *,
    ...
    if template == "service":
        ...
        # snippet. See minder.core.modules.service_template.
