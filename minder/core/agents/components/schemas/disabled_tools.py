...
Resolution (mirrors :func:`minder.core.modules.registry.load_disabled_modules`):

* The web UI persists a ``disabled_tools`` list into ``~/.minder/settings.json``.
...
def _env_disabled() -> set[str]:
    """Parse ``MINDER_DISABLED_TOOLS`` (comma/whitespace separated) into a set."""
    raw = os.environ.get("MINDER_DISABLED_TOOLS", "")
...
def load_disabled_tools() -> set[str]:
    ...
    Reads the global ``~/.minder/settings.json`` on every call (the file is tiny
...
def all_builtin_tool_meta() -> list[dict[str, str]]:
    ...
    """
    from minder.core.agents.components.schemas.definitions import _BUILTIN_TOOL_SCHEMAS
