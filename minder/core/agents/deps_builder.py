...
if TYPE_CHECKING:
    from minder.core.runtime.services.runtime_service import RuntimeSuite
    from minder.core.agents.subagents.manager import SubAgentDeps
...
def build_runtime_and_deps(
    ...
    env_overrides: dict[str, str] = {
        "MINDER_WORKSPACE": str(wd),
        "MINDER_SESSION_DIR": str(wd),
    }
    if payload.session_id:
        env_overrides["MINDER_SESSION_ID"] = payload.session_id
        env_overrides["MINDER_CONVERSATION_ID"] = payload.session_id
        env_overrides["MINDER_PROJECT_SLUG"] = f"conv-{payload.session_id}"
    ...
    if payload.blackboard_task_id:
        from minder.core.blackboard.provision import make_solver_blackboard
