...
Mirrors minder.web.bus.make_bus: one factory, one module-level singleton that
...
# it reads the ``MINDER_REDIS_URL`` env var — every process (server + worker +
# scheduler) must share the same value for the broker to function. Defaults to
# localhost for local/dev; in Docker set MINDER_REDIS_URL=redis://redis:6379/0.
broker: AsyncBroker = make_broker(
    os.environ.get("MINDER_REDIS_URL", "redis://localhost:6379/0"), result_ttl=3600
)
