...
Persists channel connections (first type: Telegram bots) to ``~/.minder/connect.json``
(0600). Mirrors the MCP config store (``minder/core/context_engineering/mcp/config.py``)
but with its own schema. Tokens live here (gitignored ~/.minder home); the UI masks them.
...
def get_connect_path() -> Path:
    """Path to the global connect.json (creates ~/.minder if needed)."""
    paths = get_paths()
