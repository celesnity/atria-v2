...
def test_cmd_query_graph_adds_docs_even_when_vector_fills_k(capsys, monkeypatch):
    """Graph-only candidates must survive the merge when vector already fills k."""
    knowledge = _load("knowledge", "ek_cli_qgraph_full")

    def _hit(doc, cls="Public", dept="COMP", score=0.9):
        cid = f"{doc}#0"
        return {
            "score": score,
            "citation": f"[{doc}]",
    ...
    class FakeStore:
        def query(self, text, k, acl_filter, department=None, mode="hybrid"):
            return [_hit(f"DOCV{i}", score=0.9 - i * 0.1) for i in range(k)]  # fills k

    class FakeGraphStore:
        def neighbors_via_entities(self, seeds, hops, acl, limit):
            return [_hit("DOCG", cls="Internal")]
    ...
    assert "DOCG" in ids, "graph-only doc was truncated by the merge cap"
    assert {f"DOCV{i}" for i in range(5)} <= ids  # vector hits all kept
