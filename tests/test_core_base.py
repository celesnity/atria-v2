...
def test_base_tool_abstract():
    ...
    from atria.core.base.abstract.base_tool import BaseTool
...
def test_tool_dependencies_dataclass():
    ...
    with patch.dict(sys.modules, {"atria.core.context_engineering.tools": MagicMock()}):
        from atria.core.base.factories.tool_factory import ToolDependencies
...
def test_tool_factory_create_registry():
    ...
    # We need to patch the module 'atria.core.context_engineering.tools' BEFORE importing ToolFactory
    ...
    if "atria.core.base.factories.tool_factory" in sys.modules:
        del sys.modules["atria.core.base.factories.tool_factory"]

    # Use patch.dict to safely modify sys.modules temporarily
    with patch.dict(sys.modules, {"atria.core.context_engineering.tools": mock_tools_module}):
        from atria.core.base.factories.tool_factory import ToolFactory, ToolDependencies
