...
Bulk-load items from CSV or JSON. A CSV needs at least
`sku,name,quantity,unit_price`; `name_vi`, `category`, `location` and
`reorder_level` are optional. New SKUs are inserted (logged as `add`); existing
SKUs are updated (a quantity change logs a `recount`). `--replace` clears all
items first. `--file -` reads from stdin (UTF-8) and `--json` emits a summary
with per-row errors — this is what the dashboard's import wizard uses:
...
python <modules>/warehouse/scripts/inventory.py import --file - --format csv --json < rows.csv
```
