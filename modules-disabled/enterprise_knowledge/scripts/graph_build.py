...
def build_extraction(
    ...
    A chunk whose extraction fails (endpoint error, non-JSON reply, truncation)
    is skipped and counted in ``failed`` — the backbone already covers it and a
    re-run retries it (failures are never cached). One bad chunk must not kill
    the whole build.
    """
    n_chunks = n_llm = n_failed = 0
    for doc in docs:
        for rec in chunk_fn(doc):
            n_chunks += 1
            ...
            if ext is None:
                ...
                except Exception as exc:  # noqa: BLE001 - skip bad chunk, keep building
                    n_failed += 1
                    print(f"[graph] extraction failed for {rec.chunk_id}: {exc}", file=sys.stderr)
                    continue
                cache.put(rec.text, ext)
                n_llm += 1
            store.upsert_extraction(rec.chunk_id, ext)
    return {"chunks": n_chunks, "llm_calls": n_llm, "failed": n_failed}
