...
- ``EK_MODEL_CTX`` — total context length (input + output). Default ``30000``.
- ``EK_<ROLE>_MAX_OUTPUT_TOKENS`` — reserved completion tokens for a role.
"""
...
def model_context_limit() -> int:
    ...
    return _int_env("EK_MODEL_CTX", 30000)


def output_tokens(role: str) -> int:
    """Reserved completion tokens for *role* (``EK_<ROLE>_MAX_OUTPUT_TOKENS``)."""
    default = _DEFAULT_OUTPUT_TOKENS.get(role, _DEFAULT_OUTPUT_FALLBACK)
    return _int_env(f"EK_{role.upper()}_MAX_OUTPUT_TOKENS", default)
