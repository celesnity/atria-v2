...
from guardrails import (  # type: ignore[import-not-found]
    ADVISORY_NOTE, PRIVACY_NOTE, enforce_citations,
)
...
def recommend(ds, user_id: str, today: date) -> list[dict]:
    ...
    def add(sid: str, reason: str, trigger: str) -> None:
        if sid in svc:
            recs.append({"service_id": sid, "service_name": svc[sid],
    ...
    if ins and ins["days_to_expiry"] is not None and ins["days_to_expiry"] <= 30:
        add("SVC001", f"Bảo hiểm TNDS sắp hết hạn ({ins['days_to_expiry']} ngày).", "insurance_expiring")
    if _int(v.get("vehicle_age_years")) > 7 and \
            str(v.get("roadside_assistance_status", "")).lower() == "inactive":
...
def _llm_answer(question: str, hits: list[dict], client) -> str:
    passages = "\n".join(f"[{h['knowledge_id']}] {h.get('answer') or h.get('question')}" for h in hits)
    system = ("Bạn trả lời câu hỏi về sở hữu xe CHỈ dựa trên các đoạn được cung cấp. "
...
def ask(ds, user_id: str, question: str, today: date, client=None) -> dict:
    ...
    if not hits:
        return {"answer": _UNCOVERED, "citations": [], "grounded": [],
                "needs_review": True, "source": "offline",
                "advisory": ADVISORY_NOTE, "privacy_note": PRIVACY_NOTE}
