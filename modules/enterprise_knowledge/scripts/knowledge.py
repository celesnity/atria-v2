...
def _augment_with_graph(
    ...
    The merge cap reserves ``EK_GRAPH_EXTRA`` slots (default 3) beyond the
    vector hits — otherwise, whenever vector retrieval already fills ``k``,
    every graph-only candidate would be truncated and ``--graph`` could only
    rerank, never add cross-document context.
    """
    try:
        gs = graph_store_obj or _build_graph_store()
        hops = int(_env("EK_GRAPH_HOPS", "1"))
        max_neighbors = int(_env("EK_GRAPH_MAX_NEIGHBORS", "20"))
        extra = int(_env("EK_GRAPH_EXTRA", "3"))
        graph_hits = graph_retrieval.expand(gs, hits, user, hops, max_neighbors)
        cap = max(k, len(hits)) + max(0, extra)
        merged = graph_retrieval.merge_hits(hits, graph_hits, cap=cap)
