---
name: stock-ops
description: Create/update/remove items and move stock — add, update, remove, receive, ship, sell, revert, adjust, move, set-reorder, mark-ordered.
---
...
Add a new item (fails if the SKU exists; logs an `add` movement). `--name-vi`
(Vietnamese display name), `--category` and `--location` are optional:

```
python <modules>/warehouse/scripts/inventory.py add \
  --sku BP-201 --name "Brake pads front" --name-vi "Má phanh trước" \
  --category Brakes --location A1-03 \
  --quantity 50 --unit-price 450000 --reorder-level 10
```
...
## Sales (multi-line, undoable)

`sell` records a sale as one `ship` movement per `--line`, all sharing a
generated `SALE-…` reference. It is all-or-nothing: if any line has
insufficient stock, nothing changes. Add `--json` to capture the reference:

```
python <modules>/warehouse/scripts/inventory.py sell --line BP-201=2 --line GO-7590=1 --json
```

`revert` undoes a whole sale or receipt by reference with compensating
movements (the ledger stays append-only). It refuses double-reverts and any
compensation that would go negative:

```
python <modules>/warehouse/scripts/inventory.py revert --reference SALE-20260705T081500Z-a3f2
```

## Reorder ("ordered") marker

Flag a low item as already reordered so it stops counting toward the restock
alert badge; `receive` clears the marker automatically when stock arrives:

```
python <modules>/warehouse/scripts/inventory.py mark-ordered --sku BP-201 --by "Ms. Lan"
python <modules>/warehouse/scripts/inventory.py unmark-ordered --sku BP-201
```
