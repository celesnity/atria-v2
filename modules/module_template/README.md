...
- **DB read overlay** — `template_db_overview` → `backend/db.py` (read-only Atria tables + mt_* writes).
...
The module shares the Atria Compose stack's `db` (PostgreSQL) and `redis` services rather than
running its own. It adds one dedicated service — MinIO — for object storage.

| Resource | Service | Notes |
|---|---|---|
| PostgreSQL | `db` (shared) | Module creates/owns `mt_*` tables; reads Atria tables read-only |
...
3. **Data** — read-only overlay of key Atria tables (agents, sessions, modules).
...
| File | Purpose |
|---|---|
| `backend/db.py` | SQLAlchemy engine; `mt_*` table definitions; read-only Atria query helpers |
...
> **This module reuses the `atria` PostgreSQL database.** It is allowed to create and write
> its own `mt_*`-prefixed tables. It may read Atria's own tables (agents, sessions, modules,
> etc.) in a **read-only** capacity for the Data panel overlay, and it must degrade gracefully
> when Atria's schema drifts (wrap Atria reads in try/except and return empty results rather
> than crashing).
>
> **Do NOT write to, alter, or drop any of Atria's tables from module code.** Violations
