    ...
    <div style={{ padding: 20 }}>
      ...
      <motion.div
        ...
        style={{
          ...
          cursor: "pointer",
          marginBottom: 20,
          background: dragging ? `${tokens.primary}08` : "transparent",
          transition: "background 0.2s",
        }}
      >
        <Upload size={24} style={{ color: tokens.primary, marginBottom: 8 }} />
        ...
        >
          <AnimatePresence>
            {media.map((m) => (
              ...
              >
                <Film size={20} style={{ color: tokens.primary, marginBottom: 8 }} />
                <a
                  href={m.url}
                  target="_blank"
                  rel="noreferrer"
                  style={{ color: tokens.text, fontSize: 12, display: "block", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", textDecoration: "none" }}
                  title={m.filename}
                >
                  {m.filename}
                </a>
                <div style={{ color: tokens.textMuted, fontSize: 11, marginTop: 4 }}>{formatSize(m.size)}</div>
              </motion.div>
            ))}
          </AnimatePresence>
        </motion.div>
