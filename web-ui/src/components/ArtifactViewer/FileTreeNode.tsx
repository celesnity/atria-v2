...
function fileIcon(ext: string) {
  ...
  if (['.md', '.mdx', '.txt', '.rst', '.org'].includes(e)) return { Icon: FileText, color: 'text-semantic-warning/80' };
  if (['.py'].includes(e)) return { Icon: Code2, color: 'text-accent-cobalt/80' };
  if (['.js', '.ts', '.tsx', '.jsx', '.mjs', '.cjs'].includes(e)) return { Icon: Code2, color: 'text-semantic-warning/80' };
  if (['.go', '.rs', '.rb', '.java', '.cpp', '.c', '.h', '.cs', '.php', '.swift', '.kt'].includes(e))
    return { Icon: Code2, color: 'text-semantic-success/80' };
  if (['.json', '.yaml', '.yml', '.toml', '.ini', '.env'].includes(e)) return { Icon: Braces, color: 'text-semantic-warning/80' };
  if (['.csv', '.tsv'].includes(e)) return { Icon: Table2, color: 'text-accent-violet/80' };
  if (['.xlsx', '.xls', '.ods'].includes(e)) return { Icon: Sheet, color: 'text-semantic-success/80' };
  if (['.png', '.jpg', '.jpeg', '.gif', '.webp', '.svg', '.ico', '.bmp'].includes(e))
    return { Icon: Image, color: 'text-accent-magenta/80' };
  if (['.pdf'].includes(e)) return { Icon: FileType2, color: 'text-semantic-danger/80' };
  if (['.ipynb'].includes(e)) return { Icon: BookOpen, color: 'text-accent-violet/80' };
  if (['.db', '.duckdb', '.sqlite', '.sqlite3'].includes(e)) return { Icon: Database, color: 'text-accent-cobalt/80' };
  if (['.html', '.htm'].includes(e)) return { Icon: Globe, color: 'text-accent-cobalt/80' };
