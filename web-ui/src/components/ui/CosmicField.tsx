...
export function CosmicField({ count = 46, className = '', wash = false }: CosmicFieldProps) {
  ...
  return (
    <div aria-hidden className={`pointer-events-none absolute inset-0 overflow-hidden ${className}`}>
      ...
      <div className="absolute -top-32 -left-24 w-[34rem] h-[34rem] rounded-[50%] bg-gradient-brand opacity-30 blur-[120px]" />
      <div className="absolute -bottom-40 -right-24 w-[30rem] h-[30rem] rounded-[50%] bg-gradient-brand opacity-20 blur-[120px]" />
      ...
      <motion.div className="absolute inset-0" style={reduce ? undefined : { x: driftX, y: driftY }}>
        {stars.map((s, i) => (
          <motion.span
            key={i}
            className="absolute rounded-[50%] bg-ink"
