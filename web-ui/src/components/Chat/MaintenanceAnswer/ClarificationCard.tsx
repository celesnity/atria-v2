...
/** Prefill the composer draft with a `field: ` prompt for the user to complete. */
function prefill(field: string) {
  const state = useChatStore.getState();
  const sid = state.currentSessionId;
  if (!sid) return;
  const prev = state.sessionStates[sid]?.draft ?? '';
  state.setDraft(sid, prev ? `${prev}\n${field}: ` : `${field}: `);
}
 ...
 */
export function ClarificationCard({ ask, missingFields, variant }: Props) {
  return (
