"""Runtime self-registration helpers for Atria service-modules.

On startup a module can call ``announce(module, cfg)`` to register itself with
the Atria host so the host knows the connector URL and can proxy tool calls.
On shutdown, ``deregister(module, cfg)`` removes the registration (best-effort).

All configuration is read from environment variables — no ``atria`` package is
imported, so this module is safe to run inside the module's own slim container.

Environment variables
---------------------
ATRIA_URL
    Base URL of the Atria host (e.g. ``http://atria:8000``). Required.
ATRIA_MODULE_CONNECTOR_URL
    Publicly reachable base URL of *this* module's connector (e.g.
    ``http://maintenance-copilot:8080``). Required.
ATRIA_MODULE_REMOTE_ENTRY
    Optional. Full URL of the Module Federation ``remoteEntry.js``.  When set,
    ``api_base`` is derived by splitting on ``/dashboard/`` so Atria knows where
    the module's dashboard lives.
KEYCLOAK_TOKEN_URL
    Optional. Token endpoint for the client-credentials grant.
ATRIA_MODULE_CLIENT_ID
    Client-ID for the client-credentials grant. Defaults to ``"atria-module"``.
ATRIA_MODULE_CLIENT_SECRET
...
class AnnounceConfig:
    ...
    atria_url: str
    """Base URL of the Atria host, e.g. ``http://atria:8000``."""
...
def resolve_announce_config() -> Optional[AnnounceConfig]:
    ...
    Returns:
        ...
        required variables (``ATRIA_URL`` and ``ATRIA_MODULE_CONNECTOR_URL``)
        are missing.
    """
    atria_url = os.environ.get("ATRIA_URL", "").rstrip("/")
    connector_url = os.environ.get("ATRIA_MODULE_CONNECTOR_URL", "").rstrip("/")

    if not atria_url or not connector_url:
        logger.debug(
            "announce: ATRIA_URL or ATRIA_MODULE_CONNECTOR_URL not set — skipping registration"
        )
        return None

    remote_entry = os.environ.get("ATRIA_MODULE_REMOTE_ENTRY") or None
    api_base: Optional[str] = None
    ...
    client_secret = os.environ.get("ATRIA_MODULE_CLIENT_SECRET") or None
...
def announce(module: str, cfg: AnnounceConfig) -> None:
    """Register this module with the Atria host.

    POSTs to ``{atria_url}/api/modules/register`` with the module name,
...
def deregister(module: str, cfg: AnnounceConfig) -> None:
    """Deregister this module from the Atria host (best-effort).

    POSTs to ``{atria_url}/api/modules/deregister``.  All errors are swallowed
    ...
    Args:
        ...
        cfg: Resolved :class:`AnnounceConfig`.
    """
    url = f"{cfg.atria_url}/api/modules/deregister"
    try:
        resp = httpx.post(
            url,
            json={"module": module},
