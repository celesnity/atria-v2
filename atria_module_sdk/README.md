...
Build an Atria **service-module** connector without hand-rolling FastAPI. The
...
It **never imports `atria`** — the connector runs in the module's own slim
container.

## Install

```bash
pip install atria-module-sdk          # once published
# or, from this repo, in the module's backend image:
pip install /path/to/atria_module_sdk
```
...
- `GET  /connector/manifest` — authoritative tool specs (Atria reconciles it)
...
Handlers can accept `principal` (the acting Atria user, forwarded as a header):
...
def signoff(body, principal):               # reachable via Atria's passthrough at
