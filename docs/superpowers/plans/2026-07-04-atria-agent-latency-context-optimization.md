# Atria Agent Latency / Context Optimization — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Cut the atria agent's per-turn latency and per-call context size by delivering+caching a stable system-prompt prefix, removing forced extra LLM round-trips, and trimming dead modules — without changing completed-turn behavior unless an opt-in flag is set.
...
**Tech Stack:** Python 3, Pydantic config (`atria/models/config.py`), pytest, `uv run pytest`, Black/Ruff (100-col), mypy strict. OpenAI-compatible HTTP via `httpx`.
...
**Files:**
- Modify: `atria/core/agents/main_agent/run_loop.py` (system-message seed at `:227-231`; payload block at `:304-307`)
- Modify: `atria/core/agents/main_agent/agent.py` (add `_compose_system_content` helper near `:198`)
...
```bash
make check
git add atria/core/agents/main_agent/agent.py atria/core/agents/main_agent/run_loop.py tests/test_system_prompt_caching.py
...
**Files:**
- Modify: `atria/models/config.py` (add `prompt_cache_key_enabled` field near `:271`)
- Modify: `atria/core/agents/main_agent/run_loop.py` (payload assembly at `:295-302`)
...
In `atria/models/config.py`, after `:271` (`plan_mode_explore_variant`), add:
...
```bash
make check
git add atria/models/config.py atria/core/agents/main_agent/run_loop.py tests/test_system_prompt_caching.py
...
**Files:**
- Modify: `atria/models/config.py` (add fields near `:271`)
...
In `atria/models/config.py` after the `prompt_cache_key_enabled` field from Task 2:
...
```bash
make check
git add atria/models/config.py tests/test_loop_config.py
...
**Files:**
- Modify: `atria/core/agents/main_agent/run_loop.py` (`:246-248`, `:261`)
...
```bash
make check
git add atria/core/agents/main_agent/run_loop.py tests/test_loop_config.py
...
**Files:**
- Modify: `atria/core/agents/main_agent/run_loop.py` (`:431-443`, `:473-485`)
...
**Files:**
- Modify: `atria/core/agents/main_agent/run_loop.py` (`:548-576`)
- Test: `tests/test_loop_config.py` (extend, source-level)
...
**Files:**
- Modify: `atria/core/agents/main_agent/run_loop.py` (`:347`)
- Test: `tests/test_loop_config.py` (extend, source-level)
...
**Files:**
- Move: `atria/core/context_engineering/retrieval/indexer.py` → `_local/dead-code/retrieval/indexer.py`
- Move: `atria/core/context_engineering/retrieval/retriever.py` → `_local/dead-code/retrieval/retriever.py`
- Modify: `atria/core/context_engineering/retrieval/__init__.py`
...
```bash
grep -rn "CodebaseIndexer\|ContextRetriever\|EntityExtractor" atria --include="*.py" | grep -v "core/context_engineering/retrieval/"
```
...
def test_token_monitor_still_importable():
    from atria.core.context_engineering.retrieval import ContextTokenMonitor  # noqa: F401


def test_dead_retrieval_symbols_gone():
    import atria.core.context_engineering.retrieval as r
...
git mv atria/core/context_engineering/retrieval/indexer.py _local/dead-code/retrieval/indexer.py
git mv atria/core/context_engineering/retrieval/retriever.py _local/dead-code/retrieval/retriever.py
```

(If `git mv` errors because files are untracked, use plain `mv`.)

Replace `atria/core/context_engineering/retrieval/__init__.py` with:
...
uv run python -c "from atria.core.context_engineering.compaction import ContextCompactor; print('ok')"
```
...
```bash
make check
git add atria/core/context_engineering/retrieval/__init__.py tests/test_dead_code_removed.py
...
**Files:**
- Modify: `atria/core/agents/components/prompts/builders.py` (`:157-162`, `:206-211`)
...
Note: if `list_tool_names` is not the registry's method name, verify with `grep -n "def list_tool" atria/core/context_engineering/tools/registry.py` and use the correct accessor; fall back to `todo_enabled = True` if the registry exposes no tool listing, to preserve current behavior when tools are present.
...
Run a real multi-tool turn through the CLI (e.g. `atria -p "read README, list the top-level atria/ folders, and summarize the architecture"`). Confirm:
...
```bash
git add atria/core/agents/components/prompts/builders.py tests/test_prompt_gating.py
