# Minder Agent — Speed / Latency / Context Optimization

**Date:** 2026-07-04
**Status:** Approved design, ready for implementation plan
**Scope:** `minder/` main agent only. The RAG / vector-search work lives in separate
...
Reduce the minder agent's per-turn latency and per-call context size by (A) enabling
...
- **ReAct loop** (`minder/core/agents/main_agent/run_loop.py`, `RunLoopMixin.run_sync`):
...
- **Dead / dormant modules:**
  - `minder/core/context_engineering/retrieval/` (`ContextRetriever`, `CodebaseIndexer`):
    built but **no live call site** in the loop.
  - `minder/core/context_engineering/memory/selector.py`: semantic scoring path exists but
...
- **`retrieval/`:** grep-confirm zero live imports/call sites, then move
  `minder/core/context_engineering/retrieval/` to `_local/` (preserve, don't delete).
