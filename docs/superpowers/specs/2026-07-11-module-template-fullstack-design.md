...
2. **Reuse the existing infra INSTANCES, isolate the module's DATA:**
   - **Postgres:** connect to the SAME `atria` database (`postgresql://atria:atria@db:5432/atria`).
     The module **owns** its tables `mt_jobs` and `mt_media` (Alembic-managed) for all
     WRITES; it **reads** Atria's own tables (`conversations`, `artifacts`) read-only for
     display/metrics. It never writes Atria's tables (avoids FK/enum/migration-drift
     footguns).
   - **Redis:** reuse the `redis` instance but a **separate DB index** (`redis://redis:6379/2`)
     for the Celery broker + result backend (Atria uses `/0`).
...
- Writing Atria's own tables from the module (explicitly rejected — read-only on core tables).
- Sharing Atria's Redis DB index or SQLAlchemy models (the module has its own engine + models).
- Replacing Atria's TaskIQ substrate (the module brings Celery for itself; it reuses only the Redis instance).
- MCP (separate track).

## The isolation caveat (documented in README + spec)

Reusing the `atria` database couples the module's read models to Atria's schema. The
module reads core tables via explicit `SELECT` (SQLAlchemy `text()` with named columns),
NOT via Atria's ORM, and treats those reads as best-effort/read-only — if a read fails
...
modules/module_template/
  backend/
    app.py            SDK connector — tools enqueue Celery jobs, CRUD mt_* rows, upload media,
                      read Atria conversations/artifacts, + the existing SDK-feature tools.
    db.py             SQLAlchemy engine (MT_DATABASE_URL → atria DB) + mt_jobs/mt_media models
                      + read helpers for Atria tables (text() SELECTs).
    ...
    alembic/          migrations creating mt_jobs, mt_media in the atria DB.
  ...
  worker/
    tasks.py          Celery tasks: process a job → update mt_jobs rows → reverse-push a live
                      progress block via AtriaClient → push_artifact on completion.
...
(celery). All join the `atria` network and reuse `db` + `redis`.

## Components

### 1. Data layer (`backend/db.py` + `alembic/`)

- SQLAlchemy 2.x sync engine from `MT_DATABASE_URL` (default `postgresql://atria:atria@db:5432/atria`).
...
- Alembic migration `0001_init` creates only `mt_jobs` + `mt_media` (never touches Atria tables).
- Read helpers (read-only on Atria tables, via `text()`): `list_conversations(limit)`,
...
- Task `run_job(job_id, session_id, steps)`: loads the `MtJob`, marks `running`, loops steps
  (update `pct` in DB + `AtriaClient().update_block(...)` for a live progress block), marks
  `done`, writes a small report to S3 + `AtriaClient().push_artifact(session_id, ...)`.
- Reverse-push uses `atria_module_sdk` (`AtriaClient`) — the worker imports the SDK, never `atria`.
...
- `template_db_overview()` — returns module counts (`mt_jobs`, `mt_media`) + read-only Atria
...
- Dashboard routes via `@conn.route`: `/jobs` (list), `/jobs/{id}` (one), `/media` (list + presigned),
  `/media/upload` (multipart → S3 + `mt_media`), `/overview` (counts + Atria reads), `/metrics`
...
- **Data** — table of module rows + read-only Atria conversations (`/connector/overview`), with
...
- Every Atria-table read is try/except → degrade (empty/zero), never 500.
...
- The module never imports `atria`; `AtriaClient`/boto3/SQLAlchemy/Celery are its own deps.
...
- Unit (module `backend/tests/`, no live infra): `db.py` model round-trips against SQLite or a
  ...
  probes. Atria-table reads tested against a stub returning rows + against a raising stub
  (degrade path).
...
- E2E (deferred to user, per CLAUDE.md, needs the compose stack + `OPENAI_API_KEY`): bring up
  ...
  and view the data/metrics panels (including the read-only Atria conversation/artifact counts).
